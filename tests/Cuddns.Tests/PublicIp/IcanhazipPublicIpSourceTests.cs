...
public class IcanhazipPublicIpSourceTests
{
    ...
    public async Task TryGetIpAsync_IPv4_HitsV4EndpointAndReturnsIp()
    {
        ...
        var sut = new IcanhazipPublicIpSource(httpClient);

        var ip = await sut.TryGetIpAsync(IpFamily.IPv4, CancellationToken.None);

        ip.Should().Be("203.0.113.10");
        handler.LastRequestUri!.ToString().Should().Be("https://ipv4.icanhazip.com/");
    }
    ...
    public async Task TryGetIpAsync_IPv6_HitsV6EndpointAndReturnsIp()
    {
        ...
        handler.LastRequestUri!.ToString().Should().Be("https://ipv6.icanhazip.com/");
    }
}
